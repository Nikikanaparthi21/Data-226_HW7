...
create or replace transient table USER_DB_HIPPO.analytics.session_summary
...
SELECT
    ...
    sessionId,
    channel
FROM USER_DB_HIPPO.raw.user_session_channel
),  __dbt__cte__session_timestamp as (
SELECT
    sessionId,
    ts
FROM USER_DB_HIPPO.raw.session_timestamp
), u AS (
